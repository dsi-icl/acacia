 ...
 * @constructor
 */
export class DynamicLargeObject extends Segment {
    _prefix: string;

    constructor(container: Container, name: string, prefix = 'default') {
     ...
     * @private
     */
    _generateSegmentName = (index = 0): string => {
     ...
     */
    createManifest = (): Promise<true | Error> => {
        const _this = this;
     ...
     * @param path {fs.PathLike} Path of the file on the disk
     ...
     */
    createFromDisk = (path: fs.PathLike, chunkSize = this.maxChunkSize): Promise<any | Error> => {
        const _this = this;
