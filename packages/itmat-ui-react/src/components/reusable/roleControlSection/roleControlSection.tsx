...
export const AddRole: React.FC<{ studyId: string, projectId?: string }> = ({ studyId, projectId }) => {
    ...
    return (
        <div className={css.add_new_role_section}>
            ...
            <br />
            <label>Name: </label>
            <input placeholder="Role name" value={inputNameString} onChange={(e) => setInputNameString(e.target.value)} />
            {' '}
            <br />
            <div className={css.add_new_role_buttons_wrapper}>
                ...
                >
                    {(addNewRole) => <button onClick={() => { setInputNameString(''); setIsExpanded(false); addNewRole({ variables: { studyId, projectId, roleName: inputNameString } }); }}>Submit</button>}
                </Mutation>
            </div>
        </div>
    );
                        ...
                        {(editRole, { loading }) => {
                            ...
                            return (
                                <div
                                    ...
                                    }} key={el} className={css.permission_selected}
                                >
                                    {el}
                                </div>
                            );
