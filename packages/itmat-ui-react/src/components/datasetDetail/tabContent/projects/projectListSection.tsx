...
export const ProjectListSection: React.FC<{ studyId: string, projectList: { id: string, name: string }[] }> = ({ studyId, projectList }) => (
    <div>
...
const OneProject: React.FC<{ studyId: string, id: string, name: string }> = ({ id, name, studyId }) => <NavLink to={`/datasets/${studyId}/projects/${id}`} activeClassName={css.active_project}><button className={`${css.project_badge} button_grey`}>{name}</button></NavLink>;


const AddNewProject: React.FC<{ studyId: string }> = ({ studyId }) => {
