    ...
    return (
        <>
            <div className={css.add_new_fields_section}>
                ...
                <h5 onClick={() => setUploadFileTabSelected(true)} className={uploadFileTabSelected ? css.selected_tab : ''}>Upload a new file</h5>
                <h5 onClick={() => setUploadFileTabSelected(false)} className={!uploadFileTabSelected ? css.selected_tab : ''}>Select from existing file</h5>
                {
    ...
    <Query<any, any> query={GET_STUDY} variables={{ studyId }}>
        {({ loading, data, error }) => {
            ...
            if (!data.getStudy || data.getStudy.files === undefined || data.getStudy.files.length === 0) {
                return (
                    <p>
                        No file has been uploaded to this dataset yet. You can do this in the
                        <NavLink to={`/datasets/${studyId}/files`}><span style={{ color: 'var(--color-primary-color)', textDecoration: 'underline' }}>file repository</span></NavLink>
                    </p>
                );
            }
...
const UploadFieldBySelectingFileForm: React.FunctionComponent<{ studyId: string, files: IFile[], dataVersionId: string, cancel: (expanded: boolean) => void }> = ({
    ...
    return (
        <div>
            ...
            <select value={selectedFile} onChange={(e) => { setSuccessfullySaved(false); setSelectedFile(e.target.value); setError(''); }}>{files.map((el: IFile) => <option key={el.id} value={el.id}>{el.fileName}</option>)}</select>
            <br />
            <br />
            <label>Tag:</label>
            <input value={tag} onChange={(e) => { setTag(e.target.value); setError(''); setSuccessfullySaved(false); }} placeholder="e.g main tree" type="text" />
            <br />
            <br />
