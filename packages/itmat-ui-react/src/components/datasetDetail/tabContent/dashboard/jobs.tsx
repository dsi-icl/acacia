...
import { SUBSCRIBE_TO_JOB_STATUS } from '@itmat/commons';
...
export const JobSection: React.FC<{ studyId: string; jobs: Array<IJobEntry<any>> }> = ({ studyId, jobs }) => {
    useSubscription(
        SUBSCRIBE_TO_JOB_STATUS,
        {
            variables: { studyId },
            onSubscriptionData: ({ client: store, subscriptionData }) => {
...
const OneJob: React.FC<{ job: IJobEntry<any> }> = ({ job }) => (
    <tr>
