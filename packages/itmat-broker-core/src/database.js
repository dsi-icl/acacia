...
class Database {
   ...
   */
    static async create(config){
        ...
        try {
            ...
            // Create indices for token collection
            const tokenCollection = db.collection(config.database.token_collection);
            await tokenCollection.createIndex({ tokenId: 1, username: 1 },
                { expireAfterSeconds: config.database.token_duration_seconds, unique: true });
            await tokenCollection.createIndex({ createdAt: 1 },
                { expireAfterSeconds: config.database.token_duration_seconds });
   ...
   */
    constructor(db, config) {
        ...
        this.dataLogCollection = db.collection(config.database.data_log_collection);
        this.fileLogCollection = db.collection(config.database.file_log_collection);
   ...
   */
    async logFileAccess(username, filename) {
        ...
        await this.fileLogCollection.insert(logEntry);
    }
   ...
   */
    async logDataAccess(username, application, key) {
        ...
        await this.dataLogCollection.insert(logEntry);
    }
   ...
   */
    async requestDataset(username, applicationID, key) {
        let id;
        try { id = new ObjectId(applicationID); } catch(error) { return null; };
        const application = await this.appCollection
            .findOne({ _id: id, users: username });
   ...
   */
    async regenerateToken(username) {
        ...
        await this.tokenCollection
            .update({ username },
                { $set: { tokenId: tokenId, username: username }, $currentDate: { createdAt: true } },
                { upsert: true });
   ...
   */
    ...
    /**
   * Deactivate the currently active access token for a given user.
   * @async
   * @param {string} username - The name of the user.
   */
    async deactivateTokenByUsername(username) {
