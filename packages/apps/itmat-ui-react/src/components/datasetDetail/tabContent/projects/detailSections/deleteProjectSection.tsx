...
export const DeleteProjectSection: React.FC<{ studyId: string; projectId: string; projectName: string }> = ({ studyId, projectId, projectName }) => {
    ...
    return (
        <>
            <p style={{ color: 'red' }}>
                Warning! This is irreversible! If you really want to delete this project, please type the name of the project (
                {projectName}
                ) below to confirm.
            </p>
            <input type="text" placeholder={projectName} value={inputText} onChange={(e) => { setInput(e.target.value); setError(''); }} />
            {' '}
            <br />
            <br />
            <Mutation<any, any>
            ...
            >
                {(deleteProject, { data, loading }) => (loading
            ...
            <button style={{ display: 'inline-block', width: '30%' }} className="button_grey" onClick={() => setIsExpanded(false)}>Cancel</button>
            <br />
