...
import { IProject, IRole, IStudy, IUserWithoutToken } from '@itmat/commons';
    ...
    Mutation: {
        ...
        createUser: async (parent: object, args: any, context: any, info: any): Promise<object> => {
            ...
            const {
                ...
                username: string; type: userTypes; realName: string; email: string; emailNotificationsActivated: boolean; password: string; description: string; organisation: string;
            } = args.user;
        ...
        editUser: async (parent: object, args: any, context: any, info: any): Promise<object> => {
            ...
            const {
                ...
                id: string; username?: string; type?: userTypes; realName?: string; email?: string; emailNotificationsActivated?: boolean; password?: string; description?: string; organisation?: string;
            } = args.user;
            if (requester.type !== userTypes.ADMIN && requester.id !== id) {
                throw new ApolloError(errorCodes.NO_PERMISSION_ERROR);
            }
            if (requester.type === userTypes.ADMIN) {
                const result: IUserWithoutToken = await db.collections!.users_collection.findOne({ id, deleted: null })!; // just an extra guard before going to bcrypt cause bcrypt is CPU intensive.
                if (result === null || result === undefined) {
                    throw new ApolloError('User not found');
                }
            }
            // if (requester.type !== userTypes.ADMIN && type !== undefined) {
