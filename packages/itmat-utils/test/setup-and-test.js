...
try {
    ...
    minio = child_process.exec('docker run --rm -p 9000:9000 minio/minio server /data');
} catch (e) {
    console.error(e);
    if (minio) {
        minio.kill();
...
const fetchpoll = setInterval(() => {
    fetch('http://localhost:9000')
        ...
        .catch(e => {
            if (e.message === 'request to http://localhost:9000/ failed, reason: connect ECONNREFUSED 127.0.0.1:9000') {
