...
import { TRPCDataCore, TRPCDataTransformationCore, TRPCFileCore, TRPCPermissionCore } from '@itmat-broker/itmat-cores';
...
const dataCore = new TRPCDataCore(db, new TRPCFileCore(db, objStore), new TRPCPermissionCore(db), utilsCore, new TRPCDataTransformationCore(utilsCore));
...
export const dataRouter = router({
    ...
    getStudyFields: baseProcedure.input(z.object({
        ...
        return await dataCore.getStudyFields(opts.ctx.user, opts.input.studyId, opts.input.versionId, opts.input.fieldIds);
    }),
    /**
     ...
     * @return IField
     */
    createStudyField: baseProcedure.input(CreateFieldInputSchema).mutation(async (opts) => {
        return await dataCore.createField(opts.ctx.user, {
     ...
     * @return IField
     */
    editStudyField: baseProcedure.input(EditFieldInputSchema).mutation(async (opts) => {
        return await dataCore.editField(opts.ctx.user, {
            studyId: opts.input.studyId,
            fieldName: opts.input.fieldName,
            fieldId: opts.input.fieldId,
            description: opts.input.description,
    ...
    deleteStudyField: baseProcedure.input(z.object({
        ...
        return await dataCore.deleteField(opts.ctx.user, opts.input.studyId, opts.input.fieldId);
    }),
    /**
    ...
    /**
         ...
         * @return Partial<IData>[] - The list of objects of Partial<IData>
         */
    getStudyDataLatest: baseProcedure.input(z.object({
        studyId: z.string(),
        fieldIds: z.optional(z.array(z.string()))
    })).query(async (opts) => {
        return await dataCore.getDataLatest(
            opts.ctx.req.user,
            opts.input.studyId,
            opts.input.fieldIds
        );
    ...
    getFiles: baseProcedure.input(z.object({
        ...
        return await dataCore.getStudyFiles(
            opts.ctx.req.user,
            opts.input.studyId,
            opts.input.fieldIds,
            opts.input.versionId
