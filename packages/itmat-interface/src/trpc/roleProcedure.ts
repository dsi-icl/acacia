...
export const roleRouter = router({
    ...
    getUserRoles: baseProcedure.input(z.object({
        ...
        return await permissionCore.getRolesOfUser(opts.ctx.user, opts.input.userId, opts.input.studyId);
    }),
    /**
     ...
     * @returns IRole[]
     */
    getStudyRoles: baseProcedure.input(z.object({
        studyId: z.string()
    })).query(async (opts) => {
        return await permissionCore.getRolesOfStudy(opts.ctx.user, opts.input.studyId);
    }),
    /**
     ...
     * @returns IRole
     */
    createStudyRole: baseProcedure.input(z.object({
        studyId: z.string(),
        name: z.string(),
        ...
        return await permissionCore.createStudyRole(
            opts.ctx.user,
            opts.input.studyId,
            opts.input.name,
     ...
     * @returns IRole
     */
    editStudyRole: baseProcedure.input(z.object({
        roleId: z.string(),
        ...
        return await permissionCore.editStudyRole(
            opts.ctx.user,
            opts.input.roleId,
            opts.input.name,
            opts.input.description,
     ...
     * @returns IRole
     */
    deleteStudyRole: baseProcedure.input(z.object({
        roleId: z.string()
    })).mutation(async (opts) => {
        return await permissionCore.deleteStudyRole(opts.ctx.user, opts.input.roleId);
    })
});
