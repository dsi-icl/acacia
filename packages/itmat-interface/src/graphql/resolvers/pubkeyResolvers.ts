...
export const pubkeyResolvers: DMPResolversMap = {
    ...
    Mutation: {
        ...
        rsaSigner: async (parent, { privateKey, message }: { privateKey: string, message: string }) => {
        ...
        issueAccessToken: async (parent, { pubkey, signature }: { pubkey: string, signature: string }) => {
        ...
        registerPubkey: async (parent, { pubkey, signature, associatedUserId }: { pubkey: string, signature: string, associatedUserId: string }, context) => {
            // refine the public-key parameter from browser
            pubkey = pubkey.replace(/\\n/g, '\n');
