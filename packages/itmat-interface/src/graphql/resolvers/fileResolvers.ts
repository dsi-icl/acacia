...
import { TRPCDataCore, TRPCDataTransformationCore, TRPCFileCore, TRPCPermissionCore, errorCodes } from '@itmat-broker/itmat-cores';
...
const dataCore = new TRPCDataCore(db, fileCore, new TRPCPermissionCore(db), utilsCore, new TRPCDataTransformationCore(utilsCore));
export const fileResolvers: DMPResolversMap = {
    ...
    Mutation: {
        uploadFile: async (_parent, args: { fileLength?: bigint, studyId: string, file: Promise<FileUpload>, description: string, hash?: string }, context) => {
            ...
            if (!targetFieldId) {
                ...
                try {
                    const res = await dataCore.uploadFileData(context.req.user, args.studyId, await args.file, targetFieldId, args.description);
                    const fileEntry = await db.collections.files_collection.findOne({ id: res.id });
                    if (args.fileLength) {
     ...
     */
        deleteFile: async (_parent, args: { fileId: string }, context) => {
            const data = await db.collections.data_collection.findOne({ value: args.fileId });
            if (!data) {
                throw new CoreError(
            ...
            return await dataCore.deleteData(context.req.user, data.studyId, data.fieldId, data.properties);
        }
    },
    Subscription: {}
};
