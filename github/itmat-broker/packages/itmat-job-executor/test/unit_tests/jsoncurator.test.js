...
describe('Unit tests for processJSONHeader function', () => {
    it('processJSONHeader function weeds out syntax error', () => {
        const exampleheader = ['1-3.3', '1-4.3'];
        const { error, parsedHeader } = processJSONHeader(exampleheader);
        expect(parsedHeader.length).toBe(2);
    ...
    it('processJSONHeader function weeds out duplicates', () => {
        const exampleheader = ['1@3.3', '1@3.3', '1@2.1', '2@3.2'];
        const { error, parsedHeader } = processJSONHeader(exampleheader);
        expect(parsedHeader.length).toBe(4);
        // expect(parsedHeader[0]).toBeNull();
        expect(parsedHeader[0]).toEqual({ fieldId: 1, timepoint: 3, measurement: 3, datatype: 'c' });
        expect(parsedHeader[1]).toEqual({ fieldId: 1, timepoint: 3, measurement: 3, datatype: 'c' });
        expect(parsedHeader[2]).toEqual({ fieldId: 1, timepoint: 2, measurement: 1, datatype: 'c' });
        expect(parsedHeader[3]).toEqual({ fieldId: 2, timepoint: 3, measurement: 2, datatype: 'c' });
        expect(error).toBeDefined();
    ...
    it('processJSONHeader function, user can annotate data type', () => {
        const exampleheader = ['1@3.3:i', '1@3.3:c', '1@2.1:b', '2@3.2:c'];
        const { error, parsedHeader } = processJSONHeader(exampleheader);
        expect(parsedHeader.length).toBe(4);
        // expect(parsedHeader[0]).toBeNull();
        expect(parsedHeader[0]).toEqual({ fieldId: 1, timepoint: 3, measurement: 3, datatype: 'i' });
        expect(parsedHeader[1]).toEqual({ fieldId: 1, timepoint: 3, measurement: 3, datatype: 'c' });
    ...
    it('processJSONHeader function weeds out wrong data type', () => {
        const exampleheader = ['1@3.3:p', '1@2.1:b', '2@3.2:e'];
        const { error, parsedHeader } = processJSONHeader(exampleheader);
        expect(parsedHeader.length).toBe(3);
        // expect(parsedHeader[0]).toBeNull();
        expect(parsedHeader[0]).toBeNull();
        expect(parsedHeader[1]).toEqual({ fieldId: 1, timepoint: 2, measurement: 1, datatype: 'b' });
    ...
    it('processJSONHeader function correctly parsed header', () => {
        const exampleheader = ['1@3.3:c', '1@2.1:i', '2@3.2:b'];
        const { error, parsedHeader } = processJSONHeader(exampleheader);
        expect(parsedHeader.length).toBe(3);
        // expect(parsedHeader[0]).toBeNull();
        expect(parsedHeader[0]).toEqual({ fieldId: 1, timepoint: 3, measurement: 3, datatype: 'c' });
        expect(parsedHeader[1]).toEqual({ fieldId: 1, timepoint: 2, measurement: 1, datatype: 'i' });
        expect(parsedHeader[2]).toEqual({ fieldId: 2, timepoint: 3, measurement: 2, datatype: 'b' });
...
describe('Unit tests for processEachSubject function', () => {
    const templateParams = {
        objectNum: 22,
        subject: [],
        parsedHeader: processJSONHeader(['1@3.3:c', '1@2.1:i', '2@3.2:b', '3@3.1:d']).parsedHeader,
    ...
    it('processEachSubject function correctly parse data row', () => {
        const { error, dataEntry } = processEachSubject({ ...templateParams, subject: {'value':['male', '95', 'true', '4.64'], 'key':'A001'} });
        expect(error).toBeUndefined();
    ...
    it('processEachSubject function weeds out datatype mismatch', () => {
        const { error, dataEntry } = processEachSubject({ ...templateParams, subject: {'value':['male', 'female', 'male', '4.64'], 'key':'A001'} });
        expect(error).toBeDefined();
        expect(error).toHaveLength(2);
        expect(error[0]).toBe("The 22 object (subjectId: A001) column 2: Cannot parse 'female' as integer.");
        expect(error[1]).toBe("The 22 object (subjectId: A001) column 3: value for boolean type must be 'true' or 'false'.");
        expect(dataEntry).toEqual({
            m_eid: 'A001',
            m_jobId: 'mockJobId',
            m_study: 'mockStudyId',
    ...
    it('processEachSubject function weeds out datatype mismatch (2)', () => {
        const { error, dataEntry } = processEachSubject({ ...templateParams, subject: {'value':['45', '53', 'false', '5a'], 'key':'A001'} });
        expect(error).toBeDefined();
        expect(error).toHaveLength(1);
        expect(error[0]).toBe("The 22 object (subjectId: A001) column 4: Cannot parse '5a' as decimal.");
        expect(dataEntry).toEqual({
            m_eid: 'A001',
            m_jobId: 'mockJobId',
            m_study: 'mockStudyId',
    ...
    it('processEachSubject function deals with missing value by skipping', () => {
        const { error, dataEntry } = processEachSubject({ ...templateParams, subject: {'value':['', '', 'false', '5.96'], 'key':'A001'} });
        expect(error).toBeUndefined();
        expect(dataEntry).toEqual({
            m_eid: 'A001',
            m_jobId: 'mockJobId',
    ...
    it('processEachSubject function deals with missing subject id correctly', () => {
        const { error, dataEntry } = processEachSubject({ ...templateParams, subject: { 'value':['male', '53', 'false', '5.3']} });
        expect(error).toBeDefined();
        expect(error).toHaveLength(1);
...
describe('JSONCuratorClass', () => {
    ...
    it('jsoncurator catches uneven field before watermark', async () => {
        ...
        const errors = await jsoncurator.processIncomingStreamAndUploadToMongo();
        expect(errors).toEqual([
            "Object sub17: Uneven field Number; expected 4 fields but got 3",
            "Object sub27: Uneven field Number; expected 4 fields but got 3"
        ]);
    ...
    it('jsoncurator catches uneven field after watermark', async () => {
        ...
        const errors = await jsoncurator.processIncomingStreamAndUploadToMongo();
        expect(errors).toEqual([
            "Object sub1530: Uneven field Number; expected 4 fields but got 2",
            "Object sub1836: Uneven field Number; expected 4 fields but got 3"
        ]);
    ...
    it('jsoncurator catches mixed errors', async () => {
        ...
        expect(errors).toEqual([
            ...
            "The 9 object (subjectId: sub7) column 2: Cannot parse 'as.d' as decimal.",
            "Object sub31: Uneven field Number; expected 4 fields but got 3",
            "Object sub32: Uneven field Number; expected 4 fields but got 3",
            "Object sub1531: Uneven field Number; expected 4 fields but got 2",
            "Object sub1837: Uneven field Number; expected 4 fields but got 3",
